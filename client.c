    ...
    while (1) {
        switch (c) {
            case 'l':   //로그인
                ...
                gotoxy(7, 3);   gets(id);         // ID 입력
                gotoxy(7, 5);   gets(pw);         // PW 입력
            ...
            case 'j':  //고객 회원가입
                send(sock, &c, 1, 0);                            // 목록코드 전달
                gotoxy(10, 3);  gets(client->id);
                gotoxy(10, 5);  gets(client->pw);
                gotoxy(10, 7);  gets(client->name);
                gotoxy(10, 9);  gets(client->num);
            ...
            case 'o':  //병원 회원가입
                send(sock, &c, 1, 0);                            // 목록코드 전달
                gotoxy(10, 3);  gets(manager->id);
                gotoxy(10, 5);  gets(manager->pw);
                gotoxy(10, 7);  gets(manager->name);
                gotoxy(10, 9);  gets(manager->lo.city); printf(" 시\t"); gets(manager->lo.dong);
